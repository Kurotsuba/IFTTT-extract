...
def read_msr_recipes(data_list, url_list):
  ...
  for each in zip(url_list, data_list):
    recipe = {
      ...
      'id': each[1][0],
      'recipe': each[1][1],
      'trigger_chan': each[1][5],
      'trigger_func': each[1][5] + '.' + each[1][6],
      'trigger_func_pure': each[1][6],
      'trigger_param': None,
      'action_chan': each[1][8],
      'action_func': each[1][8] + '.' + each[1][9],
      'action_func_pure': each[1][9],
      'action_param': None,
  ...
  for section in ['train', 'test', 'dev']:
    ...
    for i in xrange(len(data[section]['action'])):
      ...
      item = {'ids': [word_ids.get(word, len(word_ids)) for word in words],
              ...
              'words': words, 
              'user': data[section]['action'][i]['user'],
