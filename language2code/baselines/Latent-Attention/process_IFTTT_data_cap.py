...
def read_msr_recipes(data_list, url_list):
  ...
  for each in zip(url_list, data_list):
    recipe = {
      ...
      'id': each[1][0],
      'recipe': each[1][1],
      'trigger_chan': each[1][5].lower(),
      'trigger_func': each[1][5].lower() + '.' + each[1][6].lower(),
      'trigger_func_pure': each[1][6].lower(),
      'trigger_param': None,
      'action_chan': each[1][8].lower(),
      'action_func': each[1][8].lower() + '.' + each[1][9].lower(),
      'action_func_pure': each[1][9].lower(),
      'action_param': None,
