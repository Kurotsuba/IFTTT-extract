...
class IftttTrain(tf_utils.TFMainLoop):
  def __init__(self, args, config):
    ...
    self.incorrects = []
    self.prev_probs = []
    self.user_dict = pickle.load(open('../../ifttt_data/user_dict.pkl'))
  ...
  def eval_batch(self, session, mtest, batch, batch_size, get_probs=False):
    ...
    for i, e in enumerate(correct):
      if  (False in e):
        ...
        pred_item = {
          'trigger_chan': list(trigger_chans.inverse_transform([preds[i][0]]))[0],
          'trigger_func': list(trigger_funcs.inverse_transform([preds[i][1]]))[0],
          'action_chan': list(action_chans.inverse_transform([preds[i][2]]))[0],
          'action_func': list(action_funcs.inverse_transform([preds[i][3]]))[0],
        }

        try:
          fixed_pred = auto_fix.autoFix(pred_item, self.prev_probs, i, list(self.user_dict[user[i]]))
        except Exception, e:
          fixed_pred = pred_item
          
        if fixed_pred['trigger_chan'] == '500px.':
          fixed_pred['trigger_chan'] == '500px'

        try:
          fixed_pred = [
            list(trigger_chans.transform([fixed_pred['trigger_chan']]))[0],
            list(trigger_funcs.transform([fixed_pred['trigger_func']]))[0],
            list(action_chans.transform([fixed_pred['action_chan']]))[0],
            list(action_funcs.transform([fixed_pred['action_func']]))[0],
          ]
        except Exception:
          fixed_pred = pred_item

        correct[i] = (labels[i] == fixed_pred)
